...
# ----- For testing views that are restricted to logged in users. -----
import datetime
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def test_only_borrowed_books_in_list(self):
        ...
        self.assertTrue(
            'bookinstance_list' in response.context
        )
        self.assertEqual(
            len(response.context['bookinstance_list']),
            0
        )
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_forbidden_iflogged_in_but_not_correct_permission(self):
        login = self.client.login(
            username='testuser1',
            password='1X<ISRUkw+tuK'
        )
